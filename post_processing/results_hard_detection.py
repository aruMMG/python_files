...
def mean_average_precision(
    ...
    for c in range(num_classes):
        ...
        for true_box in true_boxes:
            if true_box[1] == c:
                all_ground_truths.append(true_box)


        detections = copy.deepcopy(all_detections)

        for detection in all_detections:
            # Only take out the ground_truths that have the same
            # training idx as detection
            ground_truth_img = [
                bbox for bbox in all_ground_truths if bbox[0] == detection[0]
            ]
            ...
            for idx, gt in enumerate(ground_truth_img):
                ...
                if iou and gt[7]==0:    #gt[7]== 0 easy ground truths
        ...
        if iou_threshold==0.5:
            print("length of ground_truths {} and hard ground truths {}".format(len(all_ground_truths),len(ground_truths)))
            print("len of predictions {} after removal of easy only prediction num of prediction {}".format(len(all_detections), len(detections)))
#=================================        
...
def metrics(json_path, split_class_path, target_path):
    ...
    for target_txt in glob.glob(target_path):
        ...
        for i,row in image_df.iterrows():
            ...
            pred_lis_for_sample.append(0)   # class name "o" 
        ...
        with open (target_txt) as f:
            ...
            for cnt, line in enumerate(f):
                ...
                lis_for_each_sample.append(0)   # class 
                lis_for_each_sample.append(1)   # confidence
                lis_for_each_sample.append(x1)
                lis_for_each_sample.append(y1)
                lis_for_each_sample.append(x2)
                lis_for_each_sample.append(y2)
                # print(name_only, cnt)
                class_sample_df = class_df[(class_df["img_name"]==name_only) & (class_df["split_num"]==cnt+1)]
                assert class_sample_df.shape[0]==1, "dataframe rows ambigious for {}".format(name_only)
                if class_sample_df.iloc[0]["class"]==1:
                    lis_for_each_sample.append(1)   # hard sample
                else:
                    lis_for_each_sample.append(0)   # easy sample
...
iouvs.append("mAP")
with open(os.path.join(Path(json_path).parent, "resultsAP_hardSamplesOnly.csv"), mode='w') as ap_file:
